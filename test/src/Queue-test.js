...
describe('Queue', () => {
  ...
  it('defaults to autoDelete if not passed', () => {
    const queue = Queue();
  ...
  describe('queueMessage()', () => {
    it('queues message', () => {
      ...
      expect(queue.messagesCount).to.equal(1);
    });
    ...
    it('queues second message', () => {
      ...
      expect(queue.messagesCount).to.equal(2);
    });
  });

  describe('consume()', () => {
    it('returns first pending messages', () => {
      const queue = Queue();
      ...
      expect(msgs[0]).to.have.property('pending', true);
    });

    it('with options consumerTag sets messages field', () => {
      const queue = Queue();
      queue.queueMessage({routingKey: 'test.1'});

      const msgs = queue.consume({consumerTag: 'smq.ctag-1'});
      expect(msgs[0]).to.have.property('fields').with.property('routingKey', 'test.1');
      expect(msgs[0]).to.have.property('fields').with.property('consumerTag', 'smq.ctag-1');
    });
    ...
    it('prefetch option returns messages', () => {
      ...
      const msgs = queue.consume({prefetch: 2, consumerTag: 'smq.ctag-2'});
      expect(msgs).to.have.length(2);

      expect(msgs[0]).to.have.property('fields').with.property('routingKey', 'test.1');
      expect(msgs[0]).to.have.property('fields').with.property('consumerTag', 'smq.ctag-2');
      expect(msgs[1]).to.have.property('fields').with.property('routingKey', 'test.2');
      expect(msgs[1]).to.have.property('fields').with.property('consumerTag', 'smq.ctag-2');
    });
    ...
    it('second consume returns non-pending messages', () => {
      ...
      const msgs1 = queue.consume({prefetch: 2, consumerTag: 'smq.ctag-2'});
      expect(msgs1).to.have.length(2);

      const msgs2 = queue.consume({prefetch: 3, consumerTag: 'smq.ctag-3'});
      expect(msgs2).to.have.length(3);

      expect(msgs2[0]).to.have.property('fields').with.property('routingKey', 'test.3');
      expect(msgs2[0]).to.have.property('fields').with.property('consumerTag', 'smq.ctag-3');
    });
  });
  ...
  describe('ack()', () => {
    it('consumes message', () => {
      ...
      const msg = queue.get();
      queue.ack(msg);
      expect(queue.messagesCount).to.equal(0);
    });
  ...
  describe('nack()', () => {
    ...
    it('with allUpTo requeues messages prior to current', () => {
      ...
      expect(msg).to.have.property('fields').with.property('routingKey', 'test.2');
      queue.nack(msg, true);
      expect(queue.messagesCount).to.equal(3);
    });
