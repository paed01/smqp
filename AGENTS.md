...
In `types/bundle.d.ts`, the `default` export must come before the matching named export (currently `export default Broker; export { Broker };` in that order) — dts-buddy walks the export list and the iteration order determines which gets the `_1` alias. With named first, dts-buddy collapses both into a single `export default` and the `Broker_1 as Broker` re-export at the end disappears, breaking `import { Broker } from 'smqp'`.
